"""
請助教將以下需要用到的module先安裝好
然後這邊整理所有讀寫csv檔要改路徑才能執行的部分：
156, 158, 178, 190, 201
注意一下234的地方，請先下載chromedriver，並到chrome的設定查看自己適用哪個版本，才能順利執行
請將所有路徑與這份程式碼放在同一個資料夾，謝謝!!
"""
...
class ReadTheNews(tk.Frame):
    ...
    def createWidgets(self):
        ...
        self.head.grid(row = 0, column = 0, columnspan = 4, sticky = tk.NE + tk.SW)
        self.askforid.grid(row = 1, column = 0, sticky = tk.NE + tk.SW)
        self.idInput.grid(row = 2, column = 0, sticky = tk.NE + tk.SW)
        self.askfortime.grid(row = 3, column = 0, sticky = tk.NE + tk.SW)
        self.startInput.grid(row = 4, column = 0, sticky = tk.NE + tk.SW)
        self.endInput.grid(row = 5, column = 0, sticky = tk.NE + tk.SW)
        self.run.grid(row = 6, column = 0, sticky = tk.NE)
        ...
        self.btnconfir.grid(row=1, column=3, sticky = tk.W)
        self.list.grid(row=1, column=2, sticky = tk.E)

        self.shownews_1.grid(row = 2, column = 1, columnspan = 2, sticky = tk.NE)
        self.shownews_2.grid(row = 3, column = 1, columnspan = 2, sticky = tk.NE)
        self.shownews_3.grid(row = 4, column = 1, columnspan = 2, sticky = tk.NE)
        self.shownews_4.grid(row = 5, column = 1, columnspan = 2, sticky = tk.NE)
        self.shownews_5.grid(row = 6, column = 1, columnspan = 2, sticky = tk.NE)

        self.btn1.grid(row = 2, column = 3, sticky = tk.NW)
        self.btn2.grid(row = 3, column = 3, sticky = tk.NW)
        self.btn3.grid(row = 4, column = 3, sticky = tk.NW)
        self.btn4.grid(row = 5, column = 3, sticky = tk.NW)
        self.btn5.grid(row = 6, column = 3, sticky = tk.NW)


        #在最底下顯示全文內容
        self.show = tk.Message(self, text="", anchor=tk.NW, justify="left", width=1200, padx=3, pady=2, font=f1)
        self.show.grid(row = 7, column = 0, columnspan = 4, sticky = tk.NW + tk.SE)
    ...
    def run(self):
        ...
        def find_news_url(stock, star_year, star_mon, star_day, end_year, end_mon, end_day):
            ...
            driver = webdriver.Chrome(executable_path=r"chromedriver.exe") #請先下載chromedriver並修改路徑
...
#程式開始
all_news = ["在generate_news中把抓到的新聞丟進來，所以第一篇才會是 all_news[1]"]
read = ReadTheNews()
